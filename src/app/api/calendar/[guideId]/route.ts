...
/** All days start available; any event on a day marks it booked */
function buildCalendarDays(
  ...
  for (const event of events) {
    ...
    // Walk from start to end (Google all-day end is exclusive)
  ...
  return Array.from({ length: days }, (_, i) => {
    const date = i + 1;
    return { date, status: bookedDays.has(date) ? "booked" : "available" };
  });
}
...
function mockMonthCalendar(year: number, month: number): CalendarDay[] {
  const days = daysInMonth(year, month);
  return Array.from({ length: days }, (_, i) => ({ date: i + 1, status: "available" as const }));
}
